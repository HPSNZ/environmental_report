...
q5q6q7 <- oly_df %>%
  mutate(feelsLike = heatIndex(temperature, humidity)) %>%
  group_by(year(datetime), day(datetime), venue) %>%
  summarise(maxFeelsLike = max(feelsLike)) %>%
  group_by(venue, `year(datetime)`) %>%
  mutate(over30 = round((n_distinct(`day(datetime)`[maxFeelsLike >= 30])/
         ...
         over40 = round((n_distinct(`day(datetime)`[maxFeelsLike >= 40])/
                           n_distinct(`day(datetime)`))*100, 0)) %>%
  select(`year(datetime)`, venue, over30, over35, over40) %>%
  unique() %>%
  arrange(venue)
...
q9 <- oly_df %>% 
  ...
  mutate(daysrained = round(n_distinct(`day(datetime)`[rain > 0])/
                              n_distinct(`day(datetime)`)*100, 0)) %>%
  select(`year(datetime)`, venue, daysrained) %>%
  unique()
...
Bringing together the analysis pieces for each aspect listed above we are left with the following.
...
ggplot(data = gdf_o, aes(x = Date, y = `Feels Like Temperature`)) +
  geom_line() +
  ggtitle('Tokyo Central Daily Max (Feels Like) Temperature',
          subtitle = 'Olympic Period Averaged over 2015-2020')
...
ggplot(data = gdf_p, aes(x = Date, y = `Feels Like Temperature`)) +
  geom_line() +
  ggtitle('Tokyo Central Daily Max (Feels Like) Temperature',
          subtitle = 'Paralympic Period Averaged over 2015-2020')
